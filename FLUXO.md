...
- **`/a-mao`** — Produtos em análise: histórico local + métricas via API; o **mesmo** mapa `productStatus` alimenta os botões de pipeline por produto. Em **Exportar** (coluna Ações nas tabelas Analytics), após o POST ao Spaces o fluxo regista o produto no histórico e **navega para `/a-mao`**; o export bem-sucedido marca o produto como **Conteúdo produzido** no pipeline local.
