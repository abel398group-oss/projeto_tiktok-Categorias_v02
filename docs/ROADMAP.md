...
- [x] **Dockerfile.api — Chromium (Puppeteer):** pacote Debian `chromium` + libs headless, `PUPPETEER_SKIP_DOWNLOAD=1`, `PUPPETEER_EXECUTABLE_PATH=/usr/bin/chromium`; `launchTikTokBrowser` usa `executablePath` e flags de contentor.
- [x] **Worker local + import remoto:** `npm run scraper:worker` (Puppeteer no PC) + `POST /scrape/import-remote` na API (mesma `ANALYTICS_API_KEY`); núcleo de import partilhado em `scripts/lib/import-output-core.mjs` com o CLI `db:import:output`. Fluxos `POST /scrape/run` e Docker **mantidos**. Ver ADR [0003](./adr/0003-local-scraper-worker-remote-import.md) e `docs/LOCAL_SCRAPER_WORKER.md`.
- [x] **Scrape headless — diagnóstico 0 produtos:** pós-goto `networkidle2` (reload configurável), espera + scroll, ficheiros em `extra/`, `exit 1` sem sucesso silencioso; env `SCRAPE_DIAGNOSTIC` / `SCRAPE_POST_GOTO_RELOAD` (ver `.env.example`, cabeçalho `scrapeCategory.mjs`).
- [x] **Scraper Docker — contexto BR + anti-bot (sem solver):** locale/timezone `America/Sao_Paulo`, `Accept-Language` pt-BR, `navigator.language(s)`, viewport 1366×768, UA estável (override `CHROME_STABLE_UA`), perfil persistente `.puppeteer-profile/tiktok-shop`, `puppeteer-extra-plugin-stealth` activo, pausas/scroll/rato; detecção **Security Check** → `TIKTOK_SECURITY_CHECK`, `exit 2`, diagnósticos mantidos (não tratar como coleta vazia “normal”).
