...
/**
 * Larguras de coluna editáveis (px) para `<colgroup>` — arrastar a borda direita do cabeçalho.
 * @param {number[]} defaultsPx Um valor por `<col>` (mesma ordem das células).
 */
export function useColumnWidths(defaultsPx) {
  ...
  const onGripMouseDown = useCallback((colIdx) => (e) => {
  ...
  const colElements = widths.map((w, i) => (
